    ...
    protected void init() {
        ...
        ButtonWidget featureToggleButton = this.buildButtonWidget("minecraft_access.gui.common.button.feature_toggle_button." + (MainClass.config.getConfigMap().getInventoryControlsConfigMap().isEnabled() ? "enabled" : "disabled"),
                (button) -> {
                    ...
                    button.setMessage(new LiteralText(I18n.translate("minecraft_access.gui.common.button.feature_toggle_button." + (MainClass.config.getConfigMap().getInventoryControlsConfigMap().isEnabled() ? "enabled" : "disabled"))));
                });
        ...
        ButtonWidget autoOpenRecipeBookButton = this.buildButtonWidget(
                ...
                (button) -> {
                    ...
                    MainClass.config.setConfigMap(configMap);
                    button.setMessage(new LiteralText(I18n.translate("minecraft_access.gui.common.button.toggle_button." + (MainClass.config.getConfigMap().getInventoryControlsConfigMap().isAutoOpenRecipeBook() ? "enabled" : "disabled"),
                            I18n.translate("minecraft_access.gui.inventory_controls_config_menu.button.auto_open_recipe_book")
                    )));
                });
        this.addButton(autoOpenRecipeBookButton);
        ...
        ButtonWidget rowNColumnButton = this.buildButtonWidget(
                ...
                (button) -> this.client.openScreen(new ValueEntryMenu("value_entry_menu", ValueEntryMenu.CONFIG_TYPE.INVENTORY_CONTROLS_ROW_N_COLUMN_FORMAT, this)));
        rowNColumnButton.active = false;
        this.addButton(rowNColumnButton);
        ...
        ButtonWidget delayButton = this.buildButtonWidget(
                ...
                (button) -> this.client.openScreen(new ValueEntryMenu("value_entry_menu", ValueEntryMenu.CONFIG_TYPE.INVENTORY_CONTROLS_DELAY, this)));
