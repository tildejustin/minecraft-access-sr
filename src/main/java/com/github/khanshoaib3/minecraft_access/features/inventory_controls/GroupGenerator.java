...
import com.github.khanshoaib3.minecraft_access.mixin.accessor.*;
...
//        }
        ...
        if (MinecraftClient.getInstance().player != null && screen.getHandler() instanceof EnchantmentScreenHandler enchantmentScreenHandler) {
            boolean bl = MinecraftClient.getInstance().player.abilities.creativeMode;
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                StringBuilder clueText = new StringBuilder(new TranslatableText("container.enchant.clue", enchantment.getName(l)).formatted(Formatting.WHITE).getString());
                if (!bl) {
                    if (MinecraftClient.getInstance().player.experienceLevel < k) {
                        clueText.append(new TranslatableText("container.enchant.level.requirement", enchantmentScreenHandler.enchantmentPower[j]).formatted(Formatting.RED).getString());
                    } else {
                        MutableText mutableText = m == 1 ? new TranslatableText("container.enchant.lapis.one") : new TranslatableText("container.enchant.lapis.many", m);
                        clueText.append(mutableText.formatted(i >= m ? Formatting.GRAY : Formatting.RED).getString());
                        MutableText mutableText2 = m == 1 ? new TranslatableText("container.enchant.level.one") : new TranslatableText("container.enchant.level.many", m);
        ...
        if (screen instanceof MerchantScreen merchantScreen) {
            ...
            TraderOfferList tradeOfferList = merchantScreenHandler.getRecipes();
    ...
    private static @NotNull List<SlotsGroup> inventoryAndCraftingScreensGroups(@NotNull HandledScreenAccessor screen) {
        ...
        if (!string.isEmpty()) {
            ObjectLinkedOpenHashSet<RecipeResultCollection> objectSet = new ObjectLinkedOpenHashSet<>(MinecraftClient.getInstance().getSearchableContainer(SearchManager.RECIPE_OUTPUT).findAll(string.toLowerCase(Locale.ROOT)));
    ...
    private static @NotNull List<SlotsGroup> creativeInventoryGroups(@NotNull CreativeInventoryScreen creativeInventoryScreen) {
        ...
        if (CreativeInventoryScreenAccessor.getSelectedTab() == ItemGroup.INVENTORY.getIndex()) {
